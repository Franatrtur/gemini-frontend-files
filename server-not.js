...
/********  Hack fs polyfill for Gemini server **********/
//         bringing Gemini SDK server.js functionality
//         to the browser, so you can upload files in the frontend


window.GeminiFiles = {}

function bindGeminiFile(file, fileName){
    const reader = new FileReader();
    reader.onload = function(e) {
        window.GeminiFiles[fileName] = e.target.result;
        console.log('File content loaded into window.GeminiFiles.' + fileName);
        console.log('Use "' + fileName + '" as the filePath when calling GoogleAIFileManager.uploadFile')
    };
...
(function(exports){
    ...
    // Pretends to be the Node.js's fs module
    // But just returns the previously loaded file
    const fs = {
        readFileSync(name, encoding = 'utf8') {
            return window.GeminiFiles[name]
        }
    };
